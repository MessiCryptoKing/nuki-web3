...
import fs from "fs";
export default function generator(abi, contractName, adelante) {
    const { inlineFunctions, inlineComponents, contractAddress, useTypescript, projectPath } = adelante;
    generatorGreeting();
    inProgress();
    // @ts-ignore
    const functions = abi.filter(({ type }) => type === "function").sort((a, b) => a.inputs.length > b.inputs.length ? -1 : 1);
    ...
    const functionMap = functions.map(({ name, inputs, outputs, stateMutability }) => functionTemplate(name, inputs, outputs, stateMutability, inlineFunctions, useTypescript));
    const componentMap = functions.map(({ name, inputs, outputs }) => componentTemplate(name, inputs, outputs, inlineFunctions, inlineComponents, useTypescript, true, [handleChangeArray(useTypescript, "masterLogs", "data", "none")]));
    (function genFunctions() {
        fs.mkdir(`./${projectPath}/functions`, { recursive: true }, (error) => {
            if (error)
                throw error;
            if (inlineFunctions) {
                fs.writeFile(`./${projectPath}/functions/functions${useTs(useTypescript, ".ts", ".js")}`, [inlineFunctionImport(), ...functionMap].join(""), (error) => {
                    if (error)
                        throw error;
                });
            }
            if (!inlineFunctions) {
                functionMap.map((component, index) => {
                    fs.writeFile(`./${projectPath}/functions/${functions[index].name}${useTs(useTypescript, ".ts", ".js")}`, component, (error) => {
                        if (error)
                            throw error;
                    });
                });
    ...
    (function genComponents() {
        fs.mkdir(`./${projectPath}/components`, { recursive: true }, (error) => {
            if (error)
                throw error;
            if (inlineComponents) {
                fs.writeFile(`./${projectPath}/components/components${useTs(useTypescript, ".tsx", ".jsx")}`, [inlineComponentImport(functions, inlineFunctions, useTypescript), ...componentMap].join(""), (error) => {
                    if (error)
                        throw error;
                });
            }
            if (!inlineComponents) {
                componentMap.map((component, index) => {
                    fs.writeFile(`./${projectPath}/components/${functions[index].name}${useTs(useTypescript, ".tsx", ".jsx")}`, component, (error) => {
                        if (error)
                            throw error;
                    });
                });
