...
const inputGenerator = (inputs, state) => {
    if (inputs.length == 0)
        return "";
    return `
...
export default function componentTemplate(name, inputs, outputs, inlineFunc, inline, useTs, logs, props) {
    const useState = inputs.length > 0;
    const filteredProps = propsFilter(props, outputs);
    ...
    const propsTypeMap = filteredProps.map((prop) => `${prop.name}: ${prop.type};`).join("\n  ");
    return (`${inline ? "" :
...
${inline ? "export" : "export default"} function ${capitalize(name)}(${useState || logs ? `props ${useTs ? ": Props" : ""}` : ""}) {
  ${useState || logs ? `const { ${filteredProps.map((prop) => `${prop.name}`).join(", ")} } = props` : ""}
  ${useState ?
  ...
  return (
    <div className="function-box">
      <div className="box-heading">
        ...
        <span className="text-extra"><p>${name}</p></span>${inputs.length > 0 ?
        `<p>Function inputs:</p>
          <p>(${inputs
